...
3. `Issue Tracker Client`
  - Fetches candidate issues in active states and matching dispatch label filters.
...
6. `Agent Runner`
  ...
  - Launches the local GitHub Copilot CLI session.
...
- `identifier` (string)
  - Human-readable ticket key (example: `octo-org/widgets#123`).
- `title` (string)
...
- `session_id` (string)
  - Copilot CLI ACP/headless session identifier returned by the local runtime.
- `transport` (string/enum)
  - Example: `acp_stdio`, `acp_tcp`, `headless_http`.
- `copilot_cli_pid` (string or null)
- `last_agent_event` (string/enum or null)
- `last_agent_timestamp` (timestamp or null)
- `last_agent_message` (summarized payload)
- `copilot_input_tokens` (integer)
- `copilot_output_tokens` (integer)
- `copilot_total_tokens` (integer)
...
- `copilot_totals` (aggregate tokens + runtime seconds)
- `copilot_rate_limits` (latest rate-limit snapshot from agent events)
...
- `Session ID`
  - Use the session identifier returned by the local Copilot CLI ACP/headless runtime.
...
- The software should attempt to adjust live behavior to the new config (for example polling
  cadence, concurrency limits, active/terminal states, dispatch/excluded labels, copilot settings,
...
- `copilot.command` is present and non-empty.
- `copilot.transport` is present and supported.
...
3. Fetch candidate issues from tracker using active states and label filters.
...
- If `dispatch_labels` is non-empty, it has at least one matching dispatch label.
- It has no labels listed in `excluded_labels`.
- It is not already in `running`.
- It is not already in `claimed`.
- Global concurrency slots are available.
- Per-state concurrency slots are available.
- Blocker rule for active issues passes:
  - If the issue state is active, do not dispatch when any blocker is non-terminal.
...
- For each running issue, compute `elapsed_ms` since:
  ...
  - `started_at`
- If `elapsed_ms > copilot.stall_timeout_ms`, terminate the worker and queue a retry.
...
This section defines the language-neutral contract for integrating a local GitHub Copilot CLI
runtime.
...
- Read `session_id` from the created Copilot session.
- Reuse the same `session_id` for continuation turns inside one worker run.

### 10.3 Streaming Turn Processing

The client reads transport messages until the prompt terminates.

Completion conditions:

- ACP/headless prompt completed with a normal stop reason (for example `end_turn`) -> success
- ACP/headless prompt failed -> failure
- permission or input path resolved as failure by policy -> failure
- prompt timeout (`copilot.prompt_timeout_ms`) -> failure
- subprocess exit -> failure

Continuation processing:

- If the worker decides to continue after a successful turn, it should issue another prompt on the
  same live Copilot session.
- The local Copilot CLI subprocess should remain alive across those continuation turns and be
  stopped only when the worker run is ending.

Line handling requirements:

- For `acp_stdio`, read protocol messages from stdout only.
- Buffer partial stdout lines until newline arrives.
- Attempt JSON parse on complete stdout lines.
- Stderr is not part of the ACP protocol stream:
  - ignore it or log it as diagnostics
  - do not attempt protocol JSON parsing on stderr
- For `acp_tcp` and `headless_http`, read messages from the established local endpoint using the
  supported protocol for that transport.

### 10.4 Emitted Runtime Events (Upstream to Orchestrator)

The Copilot transport client emits structured events to the orchestrator callback. Each event
should
include:

- `event` (enum/string)
- `timestamp` (UTC timestamp)
- `copilot_cli_pid` (if available)
...
### 10.5 Approval, MCP Servers, and User Input Policy

Permissions, MCP configuration, and user-input behavior are implementation-defined.

Policy requirements:

- Each implementation should document its chosen permission, tool, and operator-confirmation
  posture.
...
- The client detects this via the transport's input-required event or equivalent prompt result.

### 10.6 Timeouts and Error Mapping

Timeouts:

- `copilot.startup_timeout_ms`: timeout for starting the local CLI and establishing a session
- `copilot.prompt_timeout_ms`: total prompt stream timeout
- `copilot.stall_timeout_ms`: enforced by orchestrator based on event inactivity
...
The `Agent Runner` wraps workspace + prompt + local Copilot CLI transport client.

Behavior:

1. Create/reuse workspace for issue.
2. Build prompt from workflow template.
3. Start local Copilot CLI session.
4. Forward Copilot transport events to orchestrator.
...
1. `fetch_candidate_issues()`
  - Return issues in configured active states for a configured repository, filtered by the
    configured dispatch/excluded label policy.
...
- `tracker.repository` identifies the repository as `<owner>/<repo>`
- Candidate issue selection filters by repository owner/name, issue states, and configured
...
- Pull requests are distinct from issues and must not be normalized as issue records.
...
- Workflow-specific success often means "reached the next handoff state" (for example
  `Human Review`) rather than tracker terminal state `Closed`.
- If the local Copilot CLI uses GitHub MCP tools for tracker writes, those remain part of the agent
  ...
  - Suggested response shape:
    ...
    ```json
    {
      ...
      "running": [
        {
          "issue_id": "abc123",
          "issue_identifier": "octo-org/widgets#649",
          "state": "Open",
          "session_id": "session-1",
          "turn_count": 7,
          "last_event": "prompt_completed",
          "last_message": "",
      ...
      "retrying": [
        {
          "issue_id": "def456",
          "issue_identifier": "octo-org/widgets#650",
          "attempt": 3,
...
1. `Workflow/Config Failures`
  ...
  - Unsupported tracker kind or missing tracker credentials/repository
...
Running local Copilot CLI sessions against repositories, issue trackers, and other inputs that may contain
...
- Tightening Copilot CLI approval and sandbox settings described elsewhere in this specification instead
  of running with a maximally permissive configuration.
- Adding external isolation layers such as OS/container/VM sandboxing, network restrictions, or
  separate credentials beyond the built-in Copilot policy controls.
- Filtering which GitHub repositories, labels, milestones, projects, or other tracker sources are eligible for
  dispatch so untrusted or out-of-scope tasks do not automatically reach the agent.
- Narrowing GitHub MCP tools or other configured MCP servers so they can only read or mutate data
...
function start_service():
  ...
  state = {
    ...
    retry_attempts: {},
    completed: set(),
    copilot_totals: {input_tokens: 0, output_tokens: 0, total_tokens: 0, seconds_running: 0},
    copilot_rate_limits: null
  }
...
function dispatch_issue(issue, state, attempt):
  ...
  state.running[issue.id] = {
    ...
    issue,
    session_id: null,
    transport: config.copilot.transport,
    copilot_cli_pid: null,
...
### 16.5 Worker Attempt (Workspace + Prompt + Copilot Session)
...
function run_agent_attempt(issue, attempt, orchestrator_channel):
  ...
  while true:
    ...
    if prompt failed:
      copilot_client.stop_session(session)
      run_hook_best_effort("after_run", workspace.path)
      fail_worker("prompt error")

    prompt_result = copilot_client.run_prompt(
      session=session,
      prompt=prompt,
      issue=issue,
    ...
    if prompt_result failed:
      copilot_client.stop_session(session)
      run_hook_best_effort("after_run", workspace.path)
      fail_worker("agent prompt error")
...
- Candidate issue fetch uses active states, repository scope, and label filters
- GitHub query uses the specified repository owner/name filter
- Empty `fetch_issues_by_states([])` returns empty without API call
- Pagination preserves order across multiple pages
- Blockers are normalized from GitHub dependency data when available, or empty otherwise
- Labels are normalized to lowercase
- Dispatch labels are matched case-insensitively
- Excluded labels suppress otherwise eligible open issues
- Issue state refresh by ID returns minimal normalized issues
- Issue state refresh query uses GitHub node ID typing (`[ID!]`) as specified in Section 11.2
- Pull requests are excluded from normalized issue results
- Error mapping for request errors, non-200, GraphQL errors, malformed payloads

### 17.4 Orchestrator Dispatch, Reconciliation, and Retry

- Dispatch sort order is priority then oldest creation time
- Open issue with a matching dispatch label is eligible when other rules pass
- Open issue without a required dispatch label is not eligible
- Open issue with an excluded label is not eligible
