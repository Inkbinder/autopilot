...
async def get_issue_comments(pr_number: int) -> list[dict[str, Any]]:
    return await get_paginated_list(f"repos/{{owner}}/{{repo}}/issues/{pr_number}/comments")


async def get_review_comments(pr_number: int) -> list[dict[str, Any]]:
    return await get_paginated_list(f"repos/{{owner}}/{{repo}}/pulls/{pr_number}/comments")
...
def latest_review_request_at(comments: list[dict[str, Any]]) -> datetime | None:
    ...
    for comment in comments:
        if is_copilot_bot_user(comment.get("user", {})):
            continue
        body = comment.get("body") or ""
        if "@copilot review" not in body:
            continue
...
def is_copilot_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    return login in COPILOT_BOTS


def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_copilot_bot_user(user):
        return True
...
def is_copilot_reply_body(body: str) -> bool:
    return body.startswith("[copilot]")


def is_copilot_review_body(body: str) -> bool:
    return body.startswith("## Copilot Review")


def is_review_comment(comment: dict[str, Any]) -> bool:
    return bool(
...
def latest_copilot_issue_reply_time(comments: list[dict[str, Any]]) -> datetime | None:
    latest: datetime | None = None
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_copilot_reply_body(body):
            continue
...
def latest_copilot_reply_by_thread(comments: list[dict[str, Any]]) -> dict[int, datetime]:
...
def filter_copilot_comments(
    comments: list[dict[str, Any]],
    review_requested_at: datetime | None,
) -> list[dict[str, Any]]:
    latest_copilot_reply = latest_copilot_reply_by_thread(comments)
    latest_issue_ack = latest_copilot_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        if not is_copilot_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        created_time = comment_time(comment)
        if created_time is None:
        ...
        if review_requested_at is not None and created_time <= review_requested_at:
            continue
        threaded = is_review_comment(comment)
        if not threaded and not is_copilot_review_body(body):
            continue
        if not threaded:
            ...
            if thread_root is not None:
                last_reply = latest_copilot_reply.get(thread_root)
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_copilot_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        if is_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        if is_copilot_reply_body(body):
            continue
        if is_copilot_review_body(body):
            continue
        if "@copilot review" in body:
            continue
        created_time = comment_time(comment)
...
def filter_copilot_review_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_copilot_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_copilot_review_body(body):
            continue
        created_time = comment_time(comment)
        if latest_ack is not None and created_time is not None and created_time <= latest_ack:
            continue
...
def filter_human_review_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for comment in comments:
        ...
        thread_root = thread_root_id(comment)
        created_time = comment_time(comment)
        last_copilot_reply = None
        if thread_root is not None:
            last_copilot_reply = latest_copilot_reply.get(thread_root)
        if last_copilot_reply and created_time and created_time <= last_copilot_reply:
            continue
        filtered.append(comment)
    return filtered
...
def is_blocking_review(review: dict[str, Any], review_requested_at: datetime | None) -> bool:
    ...
    if user_login in COPILOT_BOTS:
        return state == "CHANGES_REQUESTED"
    if body.startswith("[copilot]") or state in ("APPROVED", "DISMISSED"):
        return False
    ...
    if state == "COMMENTED":
        return False
    if state:
        return state not in ("APPROVED", "DISMISSED")
    return False
...
async def wait_for_copilot(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_copilot_comments(issue_comments, review_request_at)
        bot_review_comments = filter_copilot_comments(review_comments, review_request_at)
        ...
        if bot_comments:
            latest = max(
                bot_comments,
                key=lambda comment: comment_time(comment) or datetime.min.astimezone(),
            )
            body = sanitize_terminal_output(latest.get("body") or "").strip()
            if body:
                print("Copilot left comments. Address feedback before merge.")
                print(body)
                raise SystemExit(2)
...
async def watch_pr() -> None:
    ...
    copilot_task = asyncio.create_task(wait_for_copilot(pr.number, checks_done))
    ...
    async def head_monitor() -> None:
        while True:
            ...
            if is_merge_conflicting(current):
                print(
                    "PR has merge conflicts. Resolve against main and push "
                    "before running land_watch again.",
                )
                raise SystemExit(5)
            if current.head_sha != head_sha:
                print("PR head updated; pull/amend/force-push as needed and retrigger CI")
                raise SystemExit(4)
            await asyncio.sleep(POLL_SECONDS)

    monitor_task = asyncio.create_task(head_monitor())
    success_task = asyncio.gather(copilot_task, checks_task)
