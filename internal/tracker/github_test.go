	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if after == "" {
			response["data"].(map[string]any)["search"] = map[string]any{
				...
				"nodes": []map[string]any{
					issueNodeFixture("1", 101, "Ready issue", []string{"Autopilot:Ready"}, "OPEN"),
...
func TestGitHubClientFetchIssueStatesByIDsUsesNodeQuery(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		writeTrackerJSON(t, writer, map[string]any{
			"data": map[string]any{
				"nodes": []map[string]any{
					issueNodeFixture("node-1", 41, "One", []string{"Autopilot:Ready"}, "CLOSED"),
...
func issueNodeFixture(id string, number int, title string, labels []string, state string) map[string]any {
